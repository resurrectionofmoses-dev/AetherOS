...
export const PredictiveBanner: React.FC<PredictiveBannerProps> = ({ alert, onFix, onDismiss }) => {
    ...
    return (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] w-[95%] max-w-lg animate-in slide-in-from-top-4 duration-500">
            <div className={`relative overflow-hidden rounded-xl border-2 shadow-[0_0_30px_rgba(0,0,0,0.8)] backdrop-blur-xl transition-colors duration-500 ${isError ? 'bg-red-950/95 border-red-600' : isCritical ? 'bg-orange-950/95 border-orange-500' : 'bg-amber-950/95 border-amber-500'}`}>
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
                        ...
                        <div className="min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                                <span className={`text-[7px] font-black uppercase tracking-wider px-1.5 py-px rounded border ${isError ? 'bg-red-600 text-black border-black' : isCritical ? 'bg-orange-600 text-black border-black' : 'bg-amber-600 text-black border-black'}`}>
                                    {alert.type || 'PRE-COG'}
                                </span>
                            ...
                            {alert.message ? (
                                <p className="text-[10px] text-gray-200 font-mono leading-tight">
                                    {alert.message}
                                </p>
                            ) : (
